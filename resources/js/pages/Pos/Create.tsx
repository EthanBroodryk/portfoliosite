...
export default function Create({ products }) {
  ...
  const handleScan = async (scannedBarcode) => {
    ...
    try {
      ...
      // Do NOT add locally — polling will handle it
    } catch (err) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        // Perform the action
        if (data.action === "add") addToCart(data.barcode);
        if (data.action === "remove") removeByBarcode(data.barcode);

        // Laravel Cache::pull already removes it from server,
        // so no duplicates will occur
      } catch (err) {
