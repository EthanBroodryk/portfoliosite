...
export default function JobCardPrintView({ job }: { job: JobCard }) {
  ...
  const beforePhotos =
    job.photos?.filter(photo => photo.type === "before") || [];

  const afterPhotos =
    job.photos?.filter(photo => photo.type === "after") || [];
  ...
  return (
    <div className="bg-white text-black p-6 max-w-4xl mx-auto border print:shadow-none">

      {/* ================= GLOBAL PRINT IMAGE OPTIMISATION ================= */}
      <style>{`
        @media print {
          img {
            ...
            /* 🔥 THIS reduces PDF size massively */
      ...
      {/* HEADER */}
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between border-b pb-4 mb-4 gap-4 print-tight">
        ...
        <div className="flex justify-center sm:justify-start w-full sm:w-auto">
          {job.branch?.logo && (
            <img
              src={`/storage/${job.branch.logo}`}
              alt="Logo"
              className="logo-img h-32 sm:h-56 md:h-64 w-auto max-w-[280px] object-contain"
            />
          )}
        </div>
      ...
      {/* META */}
      <div className="grid grid-cols-3 text-sm mb-4 border-y py-2">
        <p><strong>Date:</strong> {job.date || "N/A"}</p>
        <p className="text-center"><strong>Tech:</strong> {job.technician}</p>
        <p className="text-right"><strong>Order:</strong> {job.customer_order_no || "N/A"}</p>
      </div>
      ...
      <div className="text-sm space-y-2 mb-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <p><strong>Start:</strong> {formatTime(job.start_time)}</p>
          <p><strong>End:</strong> {formatTime(job.end_time)}</p>
        </div>
      ...
      <div className="text-sm space-y-1">
        <p><strong>Engine Serial:</strong> N/A</p>
        <p><strong>Engine Model:</strong> N/A</p>
        <p><strong>Run Hours:</strong> N/A</p>
      </div>

      {/* REMARKS */}
      <div className="border-t pt-4 mt-4">
        <p className="font-bold text-center">Remarks</p>
      ...
      {/* SIGNATURE */}
      <div className="border-t pt-6 flex justify-between items-end mt-6">
      ...
      {beforePhotos.length > 0 && (
        <div className="mt-6 border-t pt-4">
          <h2 className="text-lg font-bold text-center">BEFORE PHOTOS</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {beforePhotos.map((photo, i) => (
              <img
                key={i}
                src={`/storage/${photo.path}`}
                className="photo-img w-full h-40 object-cover border"
              />
            ))}
          </div>
        </div>
      ...
      {afterPhotos.length > 0 && (
        <div className="mt-6 border-t pt-4">
          <h2 className="text-lg font-bold text-center">AFTER PHOTOS</h2>
